...
This ticket captures the shared product, engineering, QA, and documentation knowledge needed to deliver `P5-MIG-001`. It is intentionally self-contained so the GitHub issue, implementation branch, review notes, and wiki updates can all trace back to the same requirements.
...
As a developer or reviewer working on the P5 milestone, I want to specify save migrations and compatibility policy so that the project can advance through the spec-driven workflow with clear scope, objective validation, and durable documentation.

## Requirements

### Functional requirements

1. The implementation or documentation change must satisfy the backlog task: **Specify save migrations and compatibility policy.**
...
- Saves from supported versions migrate or fail safely with user-facing messaging.
...
- Deliver the behavior, documentation, or planning artifact described by `P5-MIG-001`.
...
- Primary backlog item: `P5-MIG-001` from `docs/wiki/spec-driven-development-tasks.md`.
...
- Migration test suite over archived save fixtures.
