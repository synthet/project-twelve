...
This ticket captures the shared product, engineering, QA, and documentation knowledge needed to deliver `P2-LIGHT-001`. It is intentionally self-contained so the GitHub issue, implementation branch, review notes, and wiki updates can all trace back to the same requirements.
...
As a developer or reviewer working on the P2 milestone, I want to specify lighting data layout and propagation rules so that the project can advance through the spec-driven workflow with clear scope, objective validation, and durable documentation.

## Requirements

### Functional requirements

1. The implementation or documentation change must satisfy the backlog task: **Specify lighting data layout and propagation rules.**
...
- Light updates are chunk-bounded where possible and neighbor propagation is explicit.
...
- Deliver the behavior, documentation, or planning artifact described by `P2-LIGHT-001`.
...
- Primary backlog item: `P2-LIGHT-001` from `docs/wiki/spec-driven-development-tasks.md`.
...
- Lighting unit tests for edits, borders, and occlusion.
