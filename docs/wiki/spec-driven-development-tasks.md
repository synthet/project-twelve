...
## Markdown ticket registry

Each backlog row has a matching open-knowledge markdown ticket under [`tickets/`](tickets/README.md). The ticket files are the durable source for user stories, requirements, detailed technical specifications, acceptance criteria, verification plans, and GitHub project issue links. When creating or updating a GitHub issue for a task, keep the issue body linked to the markdown ticket and keep the ticket front matter synchronized with the issue URL.
