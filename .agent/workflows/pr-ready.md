...
5. **PR description** — Paste-ready Markdown with `Closes #<N>`.
...
- PR includes `Closes #<N>`; wiki ticket `Stage = Review` when applicable.
- `python3 scripts/check_paid_assets.py --staged` passes before commit.
- If `.claude/` changed, `python scripts/sync_assistant_trees.py --check` passes.
- Diff clean: no debug code, secrets, licensed art blobs, large binaries, or unrelated refactors.
