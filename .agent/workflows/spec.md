...
3. **Clarification pass**: ask only for decisions that would materially change scope, acceptance criteria, tests, or architecture; otherwise record assumptions. Confirm every user story maps to at least one `AC-n` and every `AC-n` maps to an observable outcome. Use `/clarify` when material ambiguity remains.
