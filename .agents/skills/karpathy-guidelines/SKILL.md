...
The four guardrails in [`.claude/rules/karpathy-coding.md`](../../../.claude/rules/karpathy-coding.md) are
...
4. **Success criteria** — Restate the task as a verifiable goal: the test or observable check that
   proves "done". This feeds the `/spec → /clarify → /plan → /tasks → /analyze → /implement` loop.
...
`/spec → /clarify → /plan → /tasks → /analyze → /implement → /test-and-fix → /pr-ready` loop — it sharpens each step. Use `/spec`
