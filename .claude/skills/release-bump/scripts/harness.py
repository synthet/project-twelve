...
def main(argv: list[str] | None = None) -> int:
    ...
    if source is None:
        result["notes"].append(
            "No version source found (VERSION / pyproject.toml / package.json). "
            "Ask the user whether to create VERSION + CHANGELOG.md for agent-infra releases, "
            "tag a subsystem only (e.g. tile-viz), or skip versioning. "
            "Do not invent a version file without approval. "
            "Unity bundle version in ProjectSettings is a human Editor workflow, not this harness."
        )
