...
description: Coordinate review-only Codex and Gemini runs through subagent-orchestrator MCP for ProjectTwelve, including sequential panel-style comparisons.
...
- Use only the `subagent-orchestrator` MCP server for external CLI agents (project key **`project-twelve-subagent-orchestrator`**).
