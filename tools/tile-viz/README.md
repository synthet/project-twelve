...
node src/cli.js import-mcp --file ../world-viz/mcp-dirt-stone.json --out test/fixtures/snippets/dirt-stone-reentrant-west.json
```
...
```bash
node src/cli.js render --space test/fixtures/snippets/mountain-window-corner.json \
  --assets-root ../../Assets/_Licensed/PixelFantasy/PixelTileEngine/Tiles \
  --visual-overrides out/mountain-window-corner.visual-overrides.json \
  --scale 16 --flat-light --png out/mountain-window-corner.override.png
...
| `scripts/render-all.sh` | Render all snippet fixtures + mountain capture to `out/` (bash) |
