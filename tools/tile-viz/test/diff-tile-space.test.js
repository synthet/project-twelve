...
test('diffTileSpaces identical fixture has zero diffs', () => {
  const space = loadTileSpaceFromFile(LIVE);
...
test('diffTileSpaces reports tile id changes', () => {
  const a = loadTileSpaceFromFile(LIVE);
  const b = loadTileSpaceFromFile(LIVE);
...
test('diffTileSpaces live snapshot clone summary is JSON-serializable', () => {
  const live = loadTileSpaceFromFile(LIVE);
  const clone = loadTileSpaceFromFile(LIVE);
  const result = diffTileSpaces(live, clone, { maxExamples: 3 });
